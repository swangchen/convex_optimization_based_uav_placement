...
def main():
    ...
    plt.title('优化前覆盖率 / Coverage Before Optimization')
    plt.xlabel('高度 (km) / Height (km)')
    plt.ylabel('覆盖率 / Coverage Rate')
    plt.legend()
    plt.grid(True)
    plt.subplot(1, 3, 2)
    ...
    plt.title('优化后覆盖率 / Coverage After Optimization')
    plt.xlabel('高度 (km) / Height (km)')
    plt.ylabel('覆盖率 / Coverage Rate')
    plt.legend()
    plt.grid(True)
    ...
    plt.plot(heights, coverage_pso, marker='o', label='PSO', color='green')
    plt.plot(heights, coverage_centroid, marker='o', label='Centroid-Joint', color='orange')
    plt.title('覆盖率对比 / Coverage Comparison')
    plt.xlabel('高度 (km) / Height (km)')
    plt.ylabel('覆盖率 / Coverage Rate')
    plt.legend()
    plt.grid(True)
    ...
    plt.scatter(dense_data[:, 0], dense_data[:, 1], c='black', alpha=0.1, label='数据点 / Data Points')
    plt.scatter(dense_centroids[:, 0], dense_centroids[:, 1], c='orange', marker='^', s=100,
                label='集群无人机 / Cluster UAVs')
    plt.scatter(dense_relay_positions[:, 0], dense_relay_positions[:, 1], c='green', marker='^', s=100,
                label='中继无人机 / Relay UAVs')
    plt.plot(dense_relay_positions[:, 0], dense_relay_positions[:, 1], 'o--',
             label='中继无人机轨迹 / Relay UAV Trajectory')
    plt.scatter([x_bs], [y_bs], c='red', marker='o', s=200, label='基站 / Base Station')
    base_circle = plt.Circle((x_bs, y_bs), 50, color='red', alpha=0.1, label='基站覆盖 / BS Coverage')
    plt.gca().add_patch(base_circle)
    for pos in dense_relay_positions:
        circle = plt.Circle(pos, 10, color='yellow', alpha=0.1)
        plt.gca().add_patch(circle)
    plt.title('密集场景下的三维部署与覆盖 / 3D Deployment in Dense Scenario with Coverage')
    plt.xlabel('X坐标 (m) / X Coordinate (m)')
    plt.ylabel('Y坐标 (m) / Y Coordinate (m)')
    plt.legend(loc='lower right')  # Modified line to move legend to lower-right corner
    plt.grid(True)
    ...
    plt.title('凸优化对功率损耗的优化效果 / Convex Optimization Effect on Power Loss')
    plt.xlabel('高度 (km) / Height (km)')
    plt.ylabel('功率损耗 (W) / Power Loss (W)')
    plt.legend()
    plt.grid(True)
    plt.savefig('fig7_convex_power_loss.png')
    plt.show()
    print("\n表7：凸优化对功率损耗的影响")
